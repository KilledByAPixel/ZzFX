...
```javascript
let // ZzFXMicro - Zuper Zmall Zound Zynth - v1.3.0 by Frank Force ~ 1018 bytes
zzfxV=.3,               // volume
...
R;c*=R;y*=500*d/R**3;A*=d/R;v*=d/R;z*=R;l=R*l|0;for(h=e+m+r+t+c|0;a<h;k[a++]=zzfxV*p*
f)++J%(100*F|0)||(f=q?1<q?2<q?3<q?Z((g%d)**3):M.max(M.min(M.tan(g),1),-1):1-(2*g/d%2+
2)%2:1-4*M.abs((g/d+.5|0)-g/d):Z(g),f=(l?1-B+B*Z(d*a/l):1)*(f<0?-1:1)*M.abs(f)**D*(a<
e?a/e:a<e+m?1-(a-e)/m*(1-w):a<e+m+r?w:a<h-c?(h-a-c)/t*w:0),f=c?f/2+(c>a?0:(a<h-c?1:(h
-a)/c)*k[a-c|0]/2):f,N?f=W=S*T+Q*(T=U)+P*(U=f)-Y*V-X*(V=W):0),x=(b+=u+=y)*M.cos(A*H++
),g+=x+x*E*Z(a**5),n&&++n>z&&(b+=v,C+=v,n=0),!l||++I%l||(b=C,u=G,n=n||1);p=zzfxX.
createBuffer(1,h,R);p.getChannelData(0).set(k);b=zzfxX.createBufferSource();b.buffer=
p;b.connect(zzfxX.destination);b.start();return b}
```
