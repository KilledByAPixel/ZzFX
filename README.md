...
```javascript
// ZzFXMicro - Zuper Zmall Zound Zynth - v1.1.9 by Frank Force ~ 877 bytes
zzfxV=.3    // volume
zzfx=       // play sound
...
(a)+1)%2),n&&++n>z&&(b+=v,C+=v,n=0),++I%l||(b=C,u=G,n||=1);p=zzfxX.createBuffer(
1,h,R);p.getChannelData(0).set(k);b=zzfxX.createBufferSource();b.buffer=p;b.
connect(zzfxX.destination);b.start();return b};zzfxX=new AudioContext;
```
